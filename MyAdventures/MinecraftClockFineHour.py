...
# draw hour points
for h in range(12):
    hPointAngle = (360/12) * h 
    hPointX, hPointY = findPointOnCircle(
        clockMiddle.x, clockMiddle.y,
        MIN_HAND_LENGTH, hPointAngle)
    mcdrawing.drawLine(
        clockMiddle.x, clockMiddle.y, clockMiddle.z-2,
        hPointX, hPointY, clockMiddle.z-2,
        block.IRON_BLOCK.id)

mcdrawing.drawSphere(clockMiddle.x, clockMiddle.y, clockMiddle.z, MIN_HAND_LENGTH -1,
                     block.AIR.id, 5)
